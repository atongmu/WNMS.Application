(function ($) {
    ...
    $(function () {
        ...
        uploader = WebUploader.create({
            ...
            chunkSize: '3145728',
            server: '/Sws/Sws_Station/UpLoad/1111',
        ...
        //编辑文件
        if (id != 0) {
            $.post("/Sws/Sws_Station/GetFileInfo", { id: id, classifyName: classifyName }, function (res) {
                ...
                for (var i = 0, l = res.data.length; i < l; i++) {
                    ...
                    var $item = $('<li class="upload_list"id="' + item.fileId + '">' +
                    ...
                    $item.find("span.btns").append('<a href="/Sws/Sws_Station/DownloadFile?fileId=' + item.fileId + '" ><i class="fa fa-cloud-download" title="下载"  data-value="' + item.fileId + '" ></i></a><i class="fa fa-minus-circle" title="删除"  data-value="' + item.fileId + '" ></i>');
        ...
        uploader.on('uploadSuccess', function (file, response) {
            $.post("/Sws/Sws_Station/MergeFile", { paramID: response.noticeID, fileName: file.name, ControllerName: ControllerName }, function (res) {
                $('#' + file.id).find('span.btns').append('<i class="fa fa-check-circle"></i>');
                var suffix = "."+file.ext;
                var path = "/UploadFile/" + ControllerName+"/" + response.noticeID + "/" + res.path + "/" + file.name;
                //array.push(response.attachment);
                var html = "<input type='text' name='url' hidden='hidden' value='" + path + "'/>  <input type='text' name='size' hidden='hidden' value='" + file.size + "'/>  <input type='text' name='suffix' hidden='hidden' value='" + suffix + "'/> <input type='text' name='names' hidden='hidden' value='" + file.name + "'/>";
                $("#filelist").append(html);
                $("#ID").val(response.noticeID);
            })
        });
        ...
        function DeleteFile(fileID) {
            $.post("/Sws/Sws_Station/DeleteFile", { fileId: fileID }, function (res) {
                if (res == "ok") {
                    var $li = $('#' + fileID);
                    $li.off().remove();
                } else {
