(function ($) {
    ...
    $(function () {
        ...
        //编辑文件
        if (id != 0) {
            $.post("/GD_Resource/GetFileInfo", { id: id, classifyName: classifyName }, function (res) {
                ...
                for (var i = 0, l = res.data.length; i < l; i++) {
                    ...
                    var $item = $('<li class="upload_list" id="' + item.ID + '">' +
                        '<img src = "/Content/image-upload/filetype/' + item.Suffix.replace('.', '') + '.png" style = "width:40px;height:45px;" />' +
                        '<label>' + item.FileName + '</label> <span class="btns"></span>' +
                        '</li >');

                    $item.find("span.btns").append('<i class="fa fa-check-circle"></i>');
                    $item.find("span.btns").append('<i class="fa fa-minus-circle" title="删除"  data-value="' + item.ID + '" ></i>');
        ...
        function addFile(file) {
            var $li = $('<li class="upload_list"id="' + file.id + '">' +
                '<img src = "/Content/image-upload/filetype/' + file.ext + '.png" style = "width:40px;height:45px;" />' +
        ...
        uploader.on('uploadSuccess', function (file, response) {
            $.post("/GD_Resource/MergeFile", { paramID: response.noticeID, fileName: file.name, ControllerName: ControllerName }, function (res) {
        ...
        function DeleteFile(fileID) {
            $.post("/GD_Resource/DeleteFile", { fileId: fileID }, function (res) {
                if (res == "ok") {
                    var $li = $('#' + fileID);
                    $li.off().remove();
                } else {
