...
var windowHeight = $(window).height(),
    contentWrapperHeight = windowHeight - $('.main-header').height();
$('.content-wrapper').height(contentWrapperHeight);
$('.content-iframe').height(contentWrapperHeight - $('.content-tabs').outerHeight());
...
function activeTabByPageId(pageId) {
    ...
    /**
     * 切换导航栏tab时，动态控制WebControl的关闭与显示
     */

    var iframe1 = $('.tab-pane iframe.tab_iframe');
        iframe2 = iframe1.contents().find('iframe.webControl-tab');

    // 如果视频页面已加载
    setTimeout(function() {
        iframe2 = iframe1.contents().find('iframe.webControl-tab');

        // console.log(iframe2.length);

        if(iframe2.length > 0) {
            // 将该iframe ID加入记录列表中
            hasWebControlPageIds.push(pageId);
        }
    }, 2 * 1000);

    if(iframe2.length > 0 && iframe2[0].contentWindow.webControlOpen !== undefined) {
        // console.log('视频页面已加载');

        var iframe2ContentWindow = iframe2[0].contentWindow,
            webControlOpen = iframe2ContentWindow.webControlOpen
            initPlugin = iframe2ContentWindow.initPlugin,
            closeOWebControl = iframe2ContentWindow.closeOWebControl;

        /**
         * 已存在：关闭视频
         * 不存在：初始化打开视频
         */
        //console.log(webControlOpen);
        //console.log(hasWebControlPageIds);

        if(webControlOpen) {
            // console.log('视频已打开，需要关闭');

            closeOWebControl();
        }
        else if($.inArray(pageId, hasWebControlPageIds) > -1) {
            // 如果当前页面有视频流插件，则初始化
            initPlugin();
